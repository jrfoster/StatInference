...
stat1 <- ToothGrowth %>% group_by(dose, supp) %>%
  summarize(range=paste(min(len),"-",max(len)),first=quantile(len, probs=.25), 
    third=quantile(len,probs=.75),mean=mean(len),median=median(len),var=var(len))
kable(x=stat1, digits=2, col.names=c("Dose", "Supplement", "Range", 
  "1st Quartile", "3rd Quartile", "Median", "Mean", "Variance"))
```
